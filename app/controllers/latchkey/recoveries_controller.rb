...
module Latchkey
  class RecoveriesController < ::ApplicationController
    ...
    def confirm
      result = service.consume(token: params[:token], current_session: latchkey_replacement_session,
        switch_account: params[:switch_account] == "1")
      if result.success?
        latchkey_accept(result.grant)
    ...
    def authentication_partial(partial) = "latchkey/recoveries/#{partial}"
