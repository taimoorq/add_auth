  ...
  module Rails
    module AuthenticationPages
      def latchkey_password_sign_in
        ...
        grant = Runtime.sessions.authenticate(identifier: intake, password: params[:password], replacing: latchkey_replacement_session, **latchkey_session_hints) do
      ...
      def page(partial, status: :ok)
        ...
        respond_to do |format|
          format.html { render "latchkey/sign_ins/show", layout: "latchkey/authentication", status: status }
          format.turbo_stream { render turbo_stream: turbo_stream.update("latchkey-content", partial: @page_partial), status: status }
        end
      end

      def authentication_partial(partial) = "latchkey/sign_ins/#{partial}"
