...
module Latchkey
  module Rails
    class Ejection
      MANIFEST = "config/latchkey-ejections.json"
      ...
      def files(kind:, only: "all")
        ...
        when :views
          ...
          groups.flat_map { |group| VIEW_GROUPS.fetch(group) }.flat_map do |directory|
            glob("app/views/latchkey/#{directory}/**/*.*erb")
          end.concat(glob("app/views/layouts/latchkey/**/*.*erb")).uniq.to_h { |path| [path, path] }
        when :controllers
          glob("app/controllers/latchkey/*_controller.rb").reject { |path| path.end_with?("assets_controller.rb") }.to_h { |path| [path, path] }
        when :javascript
          %w[application codec passkey].to_h do |name|
            ["app/javascript/latchkey/#{name}.js", "lib/generators/latchkey/javascript/templates/#{name}.js"]
          end.merge("app/javascript/latchkey/challenge.js" => "lib/generators/latchkey/email_link/templates/latchkey_challenge.js")
        when :mailer_views
          glob("app/views/latchkey/*_mailer/*.*erb").to_h { |path| [path, path] }
        else
      ...
      def install(kind:, only: "all")
        ...
        results = files(kind: kind, only: only).map do |destination, source|
          ...
          manifest[destination] ||= {"version" => Latchkey::VERSION, "source" => source,
      ...
      def report
        ...
        read_manifest.map do |destination, entry|
          unless inventory[destination] == entry["source"] && entry["baseline"].is_a?(String) && hash(entry["baseline"]) == entry["source_hash"]
            raise Latchkey::Error, "invalid ejection manifest entry"
          end
      ...
      def asset(name)
        destination = "app/javascript/latchkey/#{name}.js"
      ...
      def decorate(body, path)
        text = "Latchkey ejection: #{Latchkey::VERSION}; source SHA256: #{hash(body)}"
      ...
      def read_manifest
        ...
        raise Latchkey::Error, "ejection manifest is too large" if File.size(path) > 5_000_000
        value = JSON.parse(File.read(path)).fetch("files")
        raise Latchkey::Error, "invalid ejection manifest" unless value.is_a?(Hash) && value.values.all? { |entry| entry.is_a?(Hash) }
        value
      rescue JSON::ParserError, KeyError
        raise Latchkey::Error, "invalid ejection manifest"
      end
