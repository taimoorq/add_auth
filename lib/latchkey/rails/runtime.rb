...
require "uri"
require "latchkey/rails/stores/sessions"
...
module Latchkey
  module Rails
    module Runtime
      ...
      def config = Latchkey.configuration
      ...
      def security_events
        raise Latchkey::Error, "enable notifications first" unless config.notifications.enabled
        require "latchkey/rails/stores/security_events"
        Core::SecurityEvents.new(store: Stores::SecurityEvents.new(user_model: ::User, event_model: ::LatchkeySecurityEvent),
      ...
      def access_policy
        Core::AccessPolicy.new(credentials: ->(id) {
          ::LatchkeyCredential.find_by(external_id: id) if defined?(::LatchkeyCredential) && ::LatchkeyCredential.table_exists?
      ...
      def email(purpose: :sign_in)
        if purpose.to_s != "sign_in" && !config.step_up.enabled
          raise Latchkey::Error, "enable step_up first"
        end
        raise Latchkey::Error, "enable session_upgrade and email_link first" unless Core::Intake.email_available?(config)
        Core::Strategies::EmailLink.new(store: Stores::EmailTokens.new(user_model: ::User,
          token_model: ::LatchkeySignInToken, session_model: ::Session), digest: config.sign_in_token_digest,
      ...
      def passkeys
        raise Latchkey::Error, "enable passkeys and their prerequisites first" unless config.passkeys.enabled && config.step_up.enabled && config.notifications.enabled
          ...
          credential_current: ->(user:, evidence:) {
            evidence.method == :password ||
              (evidence.method == :passkey && access_policy.credential_current?(user: user, id: evidence.credential_id) &&
                ::LatchkeyCredential.find_by(external_id: evidence.credential_id)&.public_key == evidence.credential_version) ||
      ...
      def key(purpose) = ::Rails.application.key_generator.generate_key("latchkey.#{purpose}.v1", 32)
      ...
      def enqueue_email_payload(payload)
        job = ::Latchkey::EmailRequestJob.perform_later(encrypt_intake(payload))
        raise Latchkey::Error, "sign-in queue unavailable" unless job
        job
      rescue
        raise Latchkey::Error, "sign-in queue unavailable", cause: nil
      end
      ...
      def record_challenge_bypass(action:)
        ::ActiveSupport::Notifications.instrument("challenge_bypass.latchkey", action: action)
      end
      ...
      def sign_in_url(token, purpose: "sign_in")
        ...
        unless valid
          raise Latchkey::Error, "configure base_url as a fixed HTTPS origin (HTTP allowed only outside production)"
        end
