      ...
      class AccountLock
        ...
        def call(id:, additional_id: nil)
          ...
          if @users.connection.transaction_open?
            raise Latchkey::Error, "authentication must own its transaction, not run inside an outer transaction"
          end
          completed = false
          result = nil
          @users.uncached do
          ...
          raise Latchkey::Error, "authentication transaction rolled back" unless completed
          result
        end
      end
    end
