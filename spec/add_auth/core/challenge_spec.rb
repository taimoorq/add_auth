...
RSpec.describe AddAuth::Core::Challenge do
  describe AddAuth::Core::Challenge::Null do
  ...
  describe AddAuth::Core::Challenge::Test do
  ...
  describe AddAuth::Core::Challenge::Turnstile do
    let(:calls) { [] }
    let(:transport) do
  ...
  describe AddAuth::Core::Challenge::Recaptcha do
    ...
    it "keeps v2 independent from v3-only action and score fields" do
      ...
      expect(provider.verify(token: "token", remote_ip: nil, action: :sign_in)).to be_success
      expect(provider.stimulus_controller).to eq("add_auth--challenge-recaptcha")
    end
