...
RSpec.describe AddAuth::Core::StepUp do
...
RSpec.describe "Current credential policy" do
  ...
  let(:now) { Time.now }
  let(:proof) do
    AddAuth::Core::StepUp::Evidence.new(user_id: 1, session_id: 2, method: :passkey,
  ...
  it "requires a current credential verifier and reevaluates it when a grant is spent" do
    current = true
    policy = AddAuth::Core::StepUp.new(purposes: {manage_keys: {methods: [:passkey], require_passkey: true}},
