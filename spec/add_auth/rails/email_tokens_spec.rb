...
RSpec.describe AddAuth::Rails::Stores::EmailTokens, database: true do
  let!(:user) { User.create!(email_address: "person@example.test", password: "correct-password") }
  let(:store) { described_class.new(user_model: User, token_model: AddAuthSignInToken, session_model: Session) }
  let(:cipher) { AddAuth::Rails::DeliveryCipher.new(key: "d" * 32) }

  def rows = AddAuthSignInToken.order(:id).to_a
  ...
  it "never reports success when ActiveRecord swallows a rollback" do
    ...
    expect { consume_link(raw) { raise ActiveRecord::Rollback } }
      .to raise_error(AddAuth::Error, /rolled back/)
    expect(record.reload.consumed_at).to be_nil
    expect(Session.count).to eq(0)
  end
  ...
  it "rejects a finalizer's session for another user and restores the token" do
    ...
    record = issue_link
    raw = service.delivery_token(digest: record.digest)
    expect { consume_link(raw) { other.sessions.create! } }.to raise_error(AddAuth::Error, /transaction writer/)
    expect(Session.count).to eq(0)
    expect(record.reload.consumed_at).to be_nil
  end
  ...
  it "leaves one deliverable token after concurrent resends" do
    ...
    expect(AddAuthSignInToken.count).to eq(2)
    live = AddAuthSignInToken.where(revoked_at: nil, consumed_at: nil)
  ...
  it "rejects success inside an outer transaction that could subsequently roll back" do
    ...
    }.to raise_error(AddAuth::Error, /outer transaction/)
    expect(record.reload.consumed_at).to be_nil
    expect(Session.count).to eq(0)
  end
  ...
  it "requires all authentication models to share a connection pool" do
    ...
    expect {
      described_class.new(user_model: User, token_model: AddAuthSignInToken, session_model: other_pool)
  ...
  it "rejects invalid delivery encryption keys at configuration time" do
    [nil, "", "short", "x" * 33].each do |key|
      expect { AddAuth::Rails::DeliveryCipher.new(key: key) }.to raise_error(ArgumentError)
    end
  end
  it "rejects a consumed token even when the request previously cached its state" do
    record = issue_link
    raw = service.delivery_token(digest: record.digest)
    AddAuthSignInToken.cache do
  ...
  it "enforces unique digests and rolls back a colliding replacement" do
    ...
    collision_service = AddAuth::Core::Strategies::EmailLink.new(store: store, digest: digest,
...
RSpec.describe "Transaction-owned email session writer", database: true do
  ...
  def token
    service.issue(identifier: user.email_address)
    service.delivery_token(digest: AddAuthSignInToken.last.digest)
  end
  ...
  it "expires the writer when consumption commits" do
    ...
    expect { writer.call }.to raise_error(AddAuth::Error)
    expect(Session.count).to eq(1)
  end
