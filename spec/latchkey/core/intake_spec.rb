...
RSpec.describe Latchkey::Core::Intake do
  let(:digest) do
    Latchkey::Core::Digest::Hmac.new(salt: "intake", secret: "s" * 32)
  end
  ...
  it "fails closed when the provider is unavailable" do
    result = intake(challenge: Latchkey::Core::Challenge::Test.new(mode: :unavailable)).call(
  ...
  it "requires an explicit open policy to proceed during an outage and records it" do
    bypasses = []
    result = intake(challenge: Latchkey::Core::Challenge::Test.new(mode: :unavailable), policy: :open,
  ...
  it "never opens the gate for a rejected or missing token" do
    challenge = Latchkey::Core::Challenge::Test.new(mode: :rejected)
  ...
  it "rejects an unknown outage policy" do
    expect {
      intake(challenge: Latchkey::Core::Challenge::Null.new, policy: :maybe)
