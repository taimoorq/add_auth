...
RSpec.describe Latchkey::Rails::Ejection do
  around do |example|
    Dir.mktmpdir("latchkey-ejection") do |root|
      @host, @engine = File.join(root, "host"), File.join(root, "engine")
      FileUtils.mkdir_p(File.join(@engine, "app/views/latchkey/sign_ins"))
      @source = "app/views/latchkey/sign_ins/_form.html.erb"
  ...
  it "rejects forged sources and malformed manifest entries" do
    ...
    expect { ejection.report }.to raise_error(Latchkey::Error, /manifest/)
