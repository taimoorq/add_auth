...
RSpec.describe "Mail worker", database: true do
  ...
  it "retries ambiguous failure with the same bearer and never holds a DB lock over delivery" do
    ...
    raw = service.delivery_token(digest: row.digest)
    attempts = 0
    allow(Latchkey::SignInMailer).to receive(:link).and_wrap_original do |original, **arguments|
    ...
    worker = Latchkey::EmailDeliveryJob.new
  ...
  it "revokes only a permanently failed issuance while preserving a newer resend" do
    old = issue
    newer = nil
    allow(Latchkey::SignInMailer).to receive(:link) do
      newer = issue
      raise Net::SMTPFatalError, "550 recipient rejected"
    end
    Latchkey::EmailDeliveryJob.new.perform(old.id)
  ...
  it "skips revoked, consumed, expired and no-longer-eligible intents" do
    ...
    Latchkey::EmailDeliveryJob.new.perform(current.id)
    current = issue
  ...
  it "rejects attacker-controlled or insecure production link origins" do
    ...
    ["https://user@evil.test", "https://example.test/path", "https://example.test?next=evil"].each do |value|
      ...
      expect { Latchkey::Rails::Runtime.sign_in_url("token") }.to raise_error(Latchkey::Error)
    end
  ensure
    config.base_url = old
  end
  ...
  it "does not mark mail delivered when the host disables delivery" do
    row = issue
    allow(Latchkey::SignInMailer).to receive(:perform_deliveries).and_return(false)
    expect { Latchkey::EmailDeliveryJob.new.perform(row.id) }.to raise_error(Latchkey::Error, /disabled/)
    expect(row.reload.delivered_at).to be_nil
  ...
  %i[abort suppress around].each do |cancellation|
    it "terminates #{cancellation} cancellation without a false delivery or a later resend" do
      ...
      raw = service.delivery_token(digest: row.digest)
      events = []
      subscriber = ActiveSupport::Notifications.subscribe("delivery_cancelled.latchkey") { |*args| events << args.last }
      ...
      Latchkey::SignInMailer.set_callback(:deliver, kind, callback)
      Latchkey::EmailDeliveryJob.new.perform(row.id)
      expect(ActionMailer::Base.deliveries).to be_empty
      expect(row.reload.delivered_at).to be_nil
      ...
      Latchkey::SignInMailer.skip_callback(:deliver, kind, callback)
      callback = nil
      Latchkey::EmailDeliveryJob.new.perform(row.id)
      expect(ActionMailer::Base.deliveries).to be_empty
    ensure
      Latchkey::SignInMailer.skip_callback(:deliver, kind, callback) if callback
  ...
  it "refuses delivery with suppressed errors and leaves the same intent retryable" do
    ...
    Latchkey::SignInMailer.set_callback(:deliver, :before, callback)
    expect { Latchkey::EmailDeliveryJob.new.perform(row.id) }.to raise_error(Latchkey::Error, /report delivery errors/)
    expect(row.reload.delivered_at).to be_nil
    ...
    Latchkey::SignInMailer.skip_callback(:deliver, :before, callback) if callback
  end
end
