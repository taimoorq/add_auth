...
RSpec.describe "Shared rate-limit counters" do
  ...
  around do |example|
    previous = Latchkey.configuration.rate_limit_store
    Latchkey.configuration.rate_limit_store = ActiveSupport::Cache::MemoryStore.new
    example.run
  ensure
    Latchkey.configuration.rate_limit_store = previous
  end
  ...
  it "fails closed on an unavailable or unsupported cache" do
    cache = Latchkey.configuration.rate_limit_store
    allow(cache).to receive(:increment).and_return(nil)
    expect { runtime.limit(key: "account", limit: 5) }.to raise_error(Latchkey::Error, /unavailable/)
