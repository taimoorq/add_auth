...
RSpec.describe "Explicit passkey recovery", database: true do
  ...
  around do |example|
    previous = Latchkey.configuration.trusted_recovery_address
    Latchkey.configuration.trusted_recovery_address = ->(account) { account.email_address }
    example.run
  ensure
    Latchkey.configuration.trusted_recovery_address = previous
  end
  ...
  it "requires a trusted recovery address and denies strict accounts even after a reset" do
    user.update!(latchkey_strict: true)
    runtime.email(purpose: :recovery).issue(identifier: user.email_address)
    expect(LatchkeySignInToken.count).to eq(0)
    user.update!(password: "new-password")
    runtime.email(purpose: :recovery).issue(identifier: user.email_address)
    expect(LatchkeySignInToken.count).to eq(0)
    user.update!(latchkey_strict: false)
    Latchkey.configuration.trusted_recovery_address = ->(_account) {}
    runtime.email(purpose: :recovery).issue(identifier: user.email_address)
    expect(LatchkeySignInToken.count).to eq(0)
  end
