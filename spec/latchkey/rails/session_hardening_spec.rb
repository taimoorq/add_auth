...
RSpec.describe "Atomic session replacement and revocation", database: true do
  ...
  let(:service) do
    Latchkey::Core::Sessions.new(store: store, digest: Latchkey.configuration.session_token_digest,
  ...
  it "does not revoke a newer bearer generation through a stale replacement or logout snapshot" do
    ...
    old.session.update!(token_digest: Latchkey.configuration.session_token_digest.digest("new-generation"))
  ...
  it "locks opposing account switches in the same order on separate connections" do
    ...
    second = Latchkey::Rails::Runtime.sessions.start(user: second_user, method: :password)
    ...
    threads = [[second_user, first], [user, second]].map do |target, prior|
      Thread.new do
        ActiveRecord::Base.connection_pool.with_connection do
          ready << true
          go.pop
          Latchkey::Rails::Runtime.sessions.start(user: target, method: :password, replacing: prior.session)
        end
      end
    end
    2.times { ready.pop }
  ...
  it "keeps email token consumption, prior revocation and replacement creation atomic" do
    ...
    email = Latchkey::Rails::Runtime.email
    email.issue(identifier: other_user.email_address)
    token = LatchkeySignInToken.last
  ...
  it "requires a current initiating session belonging to the claimed account" do
    ...
    foreign = Latchkey::Rails::Runtime.sessions.start(user: stranger, method: :password)
