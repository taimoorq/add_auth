...
RSpec.describe Latchkey::Core::Sessions, database: true do
  let!(:user) { User.create!(email_address: "person@example.test", password: "correct-password") }
  let(:store) { Latchkey::Rails::Stores::Sessions.new(user_model: User, session_model: Session) }
  let(:digest) { Latchkey.configuration.session_token_digest }
  ...
  it "revokes every active session only with a bound fresh sign-out grant" do
    ...
    step_up = Latchkey::Core::StepUp.new(clock: clock,
      purposes: {sign_out_everywhere: {methods: [:password]}})
    evidence = Latchkey::Core::StepUp::Evidence.new(user_id: user.id, session_id: current.session.id,
  ...
  it "rotates the bearer and records a valid step-up grant atomically" do
    ...
    step_up = Latchkey::Core::StepUp.new(clock: clock,
      purposes: {manage_profile: {methods: [:password]}})
    evidence = Latchkey::Core::StepUp::Evidence.new(user_id: user.id, session_id: initial.session.id,
...
RSpec.describe "Step-up finalization races", database: true do
  ...
  let(:now) { Time.now.change(usec: 0) }
  let(:clock) { double(now: now) }
  let(:service) { Latchkey::Core::Sessions.new(store: store, digest: Latchkey.configuration.session_token_digest, eligible: Latchkey.configuration.eligible, clock: clock) }
  let(:initial) { service.start(user: user, method: :password) }

  def authorize(purpose = :manage_profile)
    proof = Latchkey::Core::StepUp::Evidence.new(user_id: user.id, session_id: initial.session.id,
      session_digest: initial.session.token_digest, credential_version: user.password_digest, method: :password, verified_at: now - 10)
    policy = Latchkey::Core::StepUp.new(clock: clock, purposes: {purpose => {methods: [:password]}})
