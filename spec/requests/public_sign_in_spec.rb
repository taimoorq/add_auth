...
RSpec.describe "Public sign in", type: :request, database: true do
  ...
  it "renders usable password/email forms and signs in without JavaScript" do
    ...
    expect(response.body).to include('autocomplete="current-password"', "turbo-cache-control", "/latchkey.css")
  ...
  it "denies another browser's POST without burning the bound delivered link" do
    previous = Latchkey.configuration.email_link.same_browser
    ...
    expect(outsider.response.body).to include('target="latchkey-content"', "Request a new link")
    ...
    Latchkey.configuration.email_link.same_browser = previous
  end
  ...
  it "uses shared stream partials and forces frame entry to a full page" do
    ...
    post "/sign-in/password", params: {email_address: user.email_address, password: "wrong"},
      headers: {"Accept" => "text/vnd.turbo-stream.html"}
    expect(response).to have_http_status(:unprocessable_entity)
    expect(response.body).to include('target="latchkey-content"', 'action="update"', "Email or password is incorrect")
  end
  ...
  it "renders one provider widget per protected form and accepts its canonical token" do
    ...
    config.challenge = Latchkey::Core::Challenge::Turnstile.new(site_key: "site", secret_key: "secret",
      transport: ->(**) {
    ...
    expect(Nokogiri::HTML(response.body).css('[data-controller="latchkey-challenge"]').size).to eq(1)
    expect(response.body).to include('data-latchkey-challenge-target="token"')
  ...
  it "keeps v2 reCAPTCHA independent from the v3 score contract" do
    config = Latchkey.configuration
    old_challenge, old_on = config.challenge, config.challenge_on
    config.challenge = Latchkey::Core::Challenge::Recaptcha.new(site_key: "site", secret_key: "secret", version: :v2,
      transport: ->(**) { [200, JSON.generate("success" => true, "hostname" => "example.test")] })
    config.challenge_on = [:email_link]

    get "/sign-in"
    expect(response.body).to include('data-latchkey-challenge-provider-value="recaptcha-v2"', "render=explicit")
  ...
  it "upgrades only Rails-verified legacy cookies and cannot reuse the old ID afterward" do
    ...
    old_deadline = Latchkey.configuration.session.legacy_bridge_until
    Latchkey.configuration.session.legacy_bridge_until = 10.minutes.from_now
    ...
    Latchkey.configuration.session.legacy_bridge_until = old_deadline
  end
  ...
  it "fails closed with useful HTML during queue and challenge outages" do
    allow(Latchkey::EmailRequestJob).to receive(:perform_later).and_return(false)
    post "/sign-in/email", params: {email_address: user.email_address}
