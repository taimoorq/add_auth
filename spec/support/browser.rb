...
%i[latchkey_chrome latchkey_no_js].each do |name|
  Capybara.register_driver name do |app|
    options = Selenium::WebDriver::Chrome::Options.new
    options.add_argument("--headless=new")
    options.add_argument("--no-sandbox")
    options.add_argument("--window-size=1280,900")
    options.add_preference("profile.managed_default_content_settings.javascript", 2) if name == :latchkey_no_js
...
module LatchkeyDetachedDocumentRead
...
Capybara::Selenium::Node.prepend(LatchkeyDetachedDocumentRead)
