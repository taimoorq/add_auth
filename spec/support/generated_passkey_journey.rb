...
require "latchkey/testing"
default = User.connection.select_all("PRAGMA table_info(users)").find { |column| column["name"] == "latchkey_strict" }.fetch("dflt_value")
abort "cross-Rails boolean default lost" unless default == "0" && User.new.latchkey_strict == false
...
client.patch "/passkeys/#{LatchkeyCredential.last.id}", params: {nickname: "Renamed laptop"}
...
link = Latchkey::Testing.delivered_link(ActionMailer::Base.deliveries.last, purpose: :recovery)
...
abort "replacement failed" unless client.response.status == 200 && LatchkeyCredential.where(user: account).count == 2
abort "recovery notification missing" unless LatchkeySecurityEvent.where(user: account, kind: "recovery_completed").last&.delivered_at
%w[/latchkey/application.js /latchkey/codec.js /latchkey/passkey.js /latchkey/challenge.js].each do |path|
  client.get path
